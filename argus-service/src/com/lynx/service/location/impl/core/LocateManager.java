...
import com.lynx.lib.location.entity.*;
...
import com.lynx.service.location.impl.util.AMapDeseder;
import com.lynx.service.location.impl.util.AMapLocateResponse;
import com.lynx.service.location.impl.util.BMapDigester;
import com.lynx.service.location.impl.util.LocationUtil;
...
public class LocateManager {
    ...
    private static final int LOC_REQ_FIN = 10;
    private static final int GPS_LOC_REQ_FIN = 1;
    private static final int NETWORK_LOC_REQ_FIN = 2;
    private static final int A_LOC_REQ_FIN = 3;
    private static final int B_LOC_REQ_FIN = 4;
                        ...
                        if (http_resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                            ...
                            if (http_entity != null) {
                                ...
                                if (!"".equals(sb.toString())) {
                                    String result = AMapDeseder.decrpyt(
                                            new AMapLocateResponse().ParseSapsXML(sb.toString()), "GBK");
                                    ...
                                    if (coord != null) {
                                        ...
                                        coord.setSource(CoordSource.AMAP);
                                        Log.d(Tag,
                                                String.format(
        ...
        private String formatRequest() {
            ...
            if (network_info != null && network_info.isAvailable()
                ...
                if (dhcp_info != null) {
                    ...
                    sb_param.append(", wifidns1: "
                            + LocationUtil.intToIpAddress(dhcp_info.dns1))
                            .append(", wifidns2: "
                                    + LocationUtil.intToIpAddress(dhcp_info.dns2))
                            .append(", wifigateway: "
                                    + LocationUtil.intToIpAddress(dhcp_info.gateway))
                            .append(", wifiipaddr: "
                                    + LocationUtil.intToIpAddress(dhcp_info.ipAddress));
                }
            }
            sb_param.append("</network>");
            ...
            if (cells != null && cells.size() > 0) {
                ...
                if (cells.get(0).type() == CellType.GSM) {
                    GSMCell gCell = (GSMCell) cells.get(0);
                    sb_cells.append("<cdma>0</cdma>");
                    sb_cells.append("<mcc>").append(gCell.getMcc())
                            .append("</mcc>");
                    sb_cells.append("<mnc>").append(gCell.getMnc())
                            .append("</mnc>");
                    sb_cells.append("<lac>").append(gCell.getLac())
                            .append("</lac>");
                    sb_cells.append("<cellid>").append(gCell.getCid())
                            .append("</cellid>");
                    sb_cells.append("<signal>").append(gCell.getAsu())
                            .append("</signal>");
                    /*
					 ...
					 * } sb_cells.append("</nb>"); }
					 */
                } else if (cells.get(0).type() == CellType.CDMA) {
                    CDMACell cdmaCell = (CDMACell) cells.get(0);
                    sb_cells.append("<cdma>1</cdma>");
                    Cell cell = cells.get(0);
                    sb_cells.append("<mcc>").append(cdmaCell.getMcc())
                            .append("</mcc>");
                    sb_cells.append("<sid>").append(cdmaCell.getSid())
                            .append("</sid>");
                    sb_cells.append("<nid>").append(cdmaCell.getNid())
                            .append("</nid>");
                    sb_cells.append("<bid>").append(cdmaCell.getBid())
                            .append("</bid>");
                    sb_cells.append("<lon>").append("").append("</lon>");
                    sb_cells.append("<lat>").append("").append("</lat>");
                    sb_cells.append("<signal>").append(0)
                            .append("</signal>");
                }
    ...
    private class BLocTask extends LocateTask {
        ...
        public void locate() {
            new Thread() {
                @Override
                public void run() {
                    CoordSource source = CoordSource.BMAP;
        ...
        public String formatRequest() {
            ...
            if (cells == null || cells.size() == 0) {
                ...
                GSMCell gsmCell = (GSMCell) cells.get(0);
                tmp += gsmCell.getMcc() + "|" + gsmCell.getMnc() + "|"
                        + gsmCell.getLac() + "|" + gsmCell.getCid() + "&clt=";
                for (int i = 0; i < cells.size(); ++i) {
                    GSMCell nbCell = (GSMCell) cells.get(i);
                    tmp += nbCell.getMcc() + "|" + nbCell.getMnc()
                            + "|" + nbCell.getLac() + "|"
                            + nbCell.getCid() + "|1;";
                }
                tmp += "10";
