...
public class DBService {
	...
	private static HashMap<String, DBService> daoMap = new HashMap<String, DBService>();

	private SQLiteDatabase db;
	private DaoConfig config;

	private DBService(DaoConfig config) throws Exception {
		if (config == null)
			throw new Exception("daoConfig is null");
		if (config.getContext() == null)
			throw new Exception("android context is null");
	...
	private synchronized static DBService getInstance(DaoConfig daoConfig) {
		DBService dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			try {
				dao = new DBService(daoConfig);
	 ...
	 */
	public static DBService create(Context context, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 *            数据库名称
	 */
	public static DBService create(Context context, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static DBService create(Context context, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 *            数据库名称
	 */
	public static DBService create(Context context, String targetDirectory, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
	 ...
	 * @return
	 */
	public static DBService create(Context context, String dbName, boolean isDebug, int dbVersion,
	 ...
	 * @return
	 */
	public static DBService create(DaoConfig daoConfig) {
	 ...
	 */
	public boolean saveBindId(Object entity) throws Exception {
		checkTableExist(entity.getClass());
	 ...
	 *            条件为空的时候，将会更新所有的数据
	 */
	public void update(Object entity, String strWhere) throws Exception {
		checkTableExist(entity.getClass());
		...
		if (SQLInfo != null) {
			...
			try {
				if (cursor.moveToNext()) {
					return CursorUtil.getEntity(cursor, clazz, this);
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
	...
	public <T> T findWithManyToOneById(Object id, Class<T> clazz) {
		...
		if (dbModel != null) {
			T entity = CursorUtil.dbModel2Entity(dbModel, clazz);
							...
							if (manyEntity != null) {
								if (many.getValue(entity).getClass() == ManyToOneLazyLoader.class) {
									if (many.getValue(entity) == null) {
										many.setValue(entity, new ManyToOneLazyLoader(entity,
												clazz, many.getManyClass(), this));
									}
	...
	private <T> List<T> findAllBySql(Class<T> clazz, String strSQL) {
		...
		try {
			...
			while (cursor.moveToNext()) {
				T t = CursorUtil.getEntity(cursor, clazz, this);
				list.add(t);
			}
			return list;
		} catch (Exception e) {
	...
	public DbModel findDbModelBySQL(String strSQL) {
		...
		try {
			if (cursor.moveToNext()) {
				return CursorUtil.getDbModel(cursor);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<DbModel> findDbModelListBySQL(String strSQL) {
		...
		try {
			while (cursor.moveToNext()) {
				dbModelList.add(CursorUtil.getDbModel(cursor));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
