...
public class ClassUtil {
	...
	public static Object getPrimaryKeyValue(Object entity) {
		return FieldUtil.getFieldValue(entity, ClassUtil.getPrimaryKeyField(entity.getClass()));
	}
	...
	public static List<Property> getPropertyList(Class<?> clazz) {
		...
		try {
			...
			for (Field f : fs) {
				// 必须是基本数据类型和没有标瞬时态的字段
				if (!FieldUtil.isTransient(f)) {
					if (FieldUtil.isBaseDateType(f)) {
						...
						property.setColumn(FieldUtil.getColumnByField(f));
						property.setFieldName(f.getName());
						property.setDataType(f.getType());
						property.setDefaultValue(FieldUtil.getPropertyDefaultValue(f));
						property.setSet(FieldUtil.getFieldSetMethod(clazz, f));
						property.setGet(FieldUtil.getFieldGetMethod(clazz, f));
						property.setField(f);
	...
	public static List<ManyToOne> getManyToOneList(Class<?> clazz) {
		...
		try {
			Field[] fs = clazz.getDeclaredFields();
			for (Field f : fs) {
				if (!FieldUtil.isTransient(f) && FieldUtil.isManyToOne(f)) {
					...
					if (f.getType() == ManyToOneLazyLoader.class) {
						Class<?> pClazz = (Class<?>) ((ParameterizedType) f.getGenericType())
					...
					mto.setColumn(FieldUtil.getColumnByField(f));
					mto.setFieldName(f.getName());
					mto.setDataType(f.getType());
					mto.setSet(FieldUtil.getFieldSetMethod(clazz, f));
					mto.setGet(FieldUtil.getFieldGetMethod(clazz, f));
	...
	public static List<OneToMany> getOneToManyList(Class<?> clazz) {
		...
		try {
			Field[] fs = clazz.getDeclaredFields();
			for (Field f : fs) {
				if (!FieldUtil.isTransient(f) && FieldUtil.isOneToMany(f)) {

					OneToMany otm = new OneToMany();

					otm.setColumn(FieldUtil.getColumnByField(f));
					...
					if (type instanceof ParameterizedType) {
						...
						throw new Exception("getOneToManyList Exception:" + f.getName()
								+ "'s type is null");
					}
					/* 修正类型赋值错误的bug，f.getClass返回的是Filed */
					otm.setDataType(f.getType());
					otm.setSet(FieldUtil.getFieldSetMethod(clazz, f));
					otm.setGet(FieldUtil.getFieldGetMethod(clazz, f));
