...
public class BarView extends View {
	...
	public BarView(Context context, AttributeSet attrs) {
		...
		rect = new Rect();
		topMargin = DisplayUtil.dip2px(context, 5);
		int textSize = DisplayUtil.sp2px(context, 15);
		barWidth = DisplayUtil.dip2px(context, 22);
		MINI_BAR_WIDTH = DisplayUtil.dip2px(context, 22);
		BAR_SIDE_MARGIN = DisplayUtil.dip2px(context, 22);
		TEXT_TOP_MARGIN = DisplayUtil.dip2px(context, 5);
		textPaint = new Paint();
	 ...
	 * 
	 * @param list
	 *            The ArrayList of Integer with the range of [0-max].
	 */
		...
		if (percentList != null && !percentList.isEmpty()) {
			for (Float f : percentList) {
				rect.set(BAR_SIDE_MARGIN * i + barWidth * (i - 1), topMargin,
						(BAR_SIDE_MARGIN + barWidth) * i, getHeight() - bottomTextHeight
								- TEXT_TOP_MARGIN);
				canvas.drawRect(rect, bgPaint);
				rect.set(BAR_SIDE_MARGIN * i + barWidth * (i - 1), topMargin
						+ (int) ((getHeight() - topMargin) * percentList.get(i - 1)),
						(BAR_SIDE_MARGIN + barWidth) * i, getHeight() - bottomTextHeight
								- TEXT_TOP_MARGIN);
