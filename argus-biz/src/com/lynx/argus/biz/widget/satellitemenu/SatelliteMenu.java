...
public class SatelliteMenu extends FrameLayout {
	...
	// ?? how to save/restore?
	private float[] degrees = new DegreeUtil().getDefaultDegrees(5, 90);
		...
		if (plusAnimationActive.compareAndSet(false, true)) {
			if (!rotated) {
				...
				for (SatelliteMenuItem item : menuItems) {
					item.getIv().startAnimation(item.getAnimOut());
				}
			} else {
				imgMain.startAnimation(mainRotateRight);
				for (SatelliteMenuItem item : menuItems) {
					item.getIv().startAnimation(item.getAnimIn());
				}
			}
			rotated = !rotated;
		}
	...
	public void addItems(List<SatelliteMenuItem> items) {
		...
		for (SatelliteMenuItem menuItem : menuItems) {
			...
			Animation itemIn = SatelliteAnimationUtil.createItemInAnimation(getContext(), index,
					expandDuration, finalX, finalY);
	...
	private float[] getDegrees(int count) {
		return DegreeUtil.getDefaultDegrees(count, totalSpacingDegree);
	}

	private void recalculateMeasureDiff() {
		int itemWidth = 0;
		if (menuItems.size() > 0) {
			itemWidth = menuItems.get(0).getIv().getWidth();
		}
		...
		public void onAnimationStart(Animation animation) {
			if (viewRef != null) {
				...
				if (view != null) {
					...
					if (isInAnimation) {
						menuItem.getIv().setVisibility(View.VISIBLE);
						menuItem.getIvClone().setVisibility(View.GONE);
					} else {
						menuItem.getIvClone().setVisibility(View.GONE);
						menuItem.getIv().setVisibility(View.VISIBLE);
					}
		...
		public void onAnimationEnd(Animation animation) {
			if (viewRef != null) {
				...
				if (view != null) {
					...
					if (isInAnimation) {
						menuItem.getIv().setVisibility(View.GONE);
						menuItem.getIvClone().setVisibility(View.GONE);
					} else {
						menuItem.getIvClone().setVisibility(View.VISIBLE);
						menuItem.getIv().setVisibility(View.GONE);
					}
				}
			}
	...
	private static class InternalSatelliteOnClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			if (menu != null) {
				...
				v.startAnimation(menuItem.getAnimClick());
			}
		}
	}
	...
	/**
	 * Defines the total space between the initial and final item in degrees. Note: Calling before adding items is
	...
	/**
	 * Sets the duration for expanding and collapsing the items in miliseconds. Note: Calling before adding items is
	 * strongly recommended.
	 * 
