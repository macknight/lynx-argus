...
public class SysInfoFragment extends LFFragment {
	...
	static {
		...
		SYS_INFO.add(new BasicNameValuePair("sdk",
				android.os.Build.VERSION.SDK_INT + ""));
		SYS_INFO.add(new BasicNameValuePair("version_release",
		...
		SYS_INFO.add(new BasicNameValuePair("finger_print",
				android.os.Build.FINGERPRINT));
		SYS_INFO.add(new BasicNameValuePair("id", android.os.Build.ID));
		SYS_INFO.add(new BasicNameValuePair("manufacturer",
	...
	private void addSysInfo(View parent) {
		...
		if (coords != null && coords.size() > 0) {
			for (int i = 0; i < coords.size(); ++i) {
				...
				addRow(tlCoordInfo, data, i == coords.size() - 1, i,
						coord_weights);
			}
		}
	}
	...
	private void addRow(TableLayout parent, List<String> data, boolean islst,
		...
		parent.addView(tr, new TableLayout.LayoutParams(
				ViewGroup.LayoutParams.WRAP_CONTENT,
				ViewGroup.LayoutParams.MATCH_PARENT));
	}
